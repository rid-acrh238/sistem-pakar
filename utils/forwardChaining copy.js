import { pool } from '../src/config/db.js';

/**
 * Forward Chaining + PHQ-9 Scoring
 * jawabanUser: { C1: 2, C2: 0, ... }
 */

/**
 * Forward Chaining Hybrid + PHQ-9
 * jawabanUser: { C1: 2, C2: 0, ... }
 */

export async function forwardChaining(jawabanUser) {
    try {
        // =========================
        // 1. FAKTA & SKOR PHQ-9
        // =========================
        const fakta = [];
        let skorPHQ9 = 0;
        let adaNiatBunuhDiri = false;

        for (const [kode, skor] of Object.entries(jawabanUser)) {
            const nilai = parseInt(skor);
            skorPHQ9 += nilai;

            if (nilai > 0) fakta.push(kode);
            if (kode === 'C9' && nilai > 0) adaNiatBunuhDiri = true;
        }

        // =========================
        // 2. LEVEL DASAR PHQ-9
        // =========================
        let levelPHQ9 = '';
        if (skorPHQ9 <= 4) levelPHQ9 = 'Minimal / Normal';
        else if (skorPHQ9 <= 9) levelPHQ9 = 'Depresi Ringan';
        else if (skorPHQ9 <= 14) levelPHQ9 = 'Depresi Sedang';
        else if (skorPHQ9 <= 19) levelPHQ9 = 'Depresi Berat';
        else levelPHQ9 = 'Depresi Berat';

        // =========================
        // 3. BASIS ATURAN (RULES)
        // =========================
        const rules = [
            {
                nama: 'Depresi Ringan',
                gejala: ['C1','C2','C3'],
                minSkor: 5
            },
            {
                nama: 'Depresi Sedang',
                gejala: ['C1','C2','C3','C4','C6'],
                minSkor: 10
            },
            {
                nama: 'Depresi Mayor Berat',
                gejala: ['C1','C2','C3','C4','C5','C6','C7','C8','C9'],
                minSkor: 15
            }
        ];

        // =========================
        // 4. FORWARD CHAINING BERBOBOT
        // =========================
        let hasilAturan = null;

        for (const rule of rules) {
            const cocokGejala = rule.gejala.every(g => fakta.includes(g));
            const skorCukup = skorPHQ9 >= rule.minSkor;

            if (cocokGejala && skorCukup) {
                hasilAturan = rule; // lanjut, cari yang paling berat
            }
        }

        // =========================
        // 5. DIAGNOSIS AKHIR (HYBRID)
        // =========================
        let diagnosisFinal = levelPHQ9;
        let saran = 'Pantau kondisi dan jaga kesehatan mental Anda.';

        if (hasilAturan) {
            diagnosisFinal = hasilAturan.nama;
        }

        // =========================
        // 6. SAFETY OVERRIDE (BUKAN NAIK LEVEL)
        // =========================
        if (adaNiatBunuhDiri) {
            diagnosisFinal += ' (PERLU PERHATIAN KHUSUS)';
            saran =
                '⚠️ Anda mengindikasikan pemikiran menyakiti diri. ' +
                'Hasil ini adalah peringatan, bukan vonis. ' +
                'Disarankan segera menghubungi profesional atau orang tepercaya.';
        } else {
            // saran normal sesuai level
            if (diagnosisFinal.includes('Ringan')) {
                saran = 'Disarankan relaksasi, istirahat cukup, dan pemantauan mandiri.';
            } else if (diagnosisFinal.includes('Sedang')) {
                saran = 'Gejala mulai mengganggu. Disarankan berkonsultasi dengan profesional.';
            } else if (diagnosisFinal.includes('Berat')) {
                saran = 'Gejala signifikan. Sangat disarankan mencari bantuan profesional.';
            }
        }

        // =========================
        // 7. OUTPUT FINAL
        // =========================
        return {
            skor: skorPHQ9,
            fakta,
            diagnosis: diagnosisFinal,
            aturan_dipakai: hasilAturan ? hasilAturan.gejala : [],
            saran
        };

    } catch (err) {
        console.error(err);
        return {
            skor: 0,
            fakta: [],
            diagnosis: 'Error sistem',
            aturan_dipakai: [],
            saran: '-'
        };
    }
}



// export async function forwardChaining(jawabanUser) {
//     try {
//         // =========================
//         // 1. UBAH JAWABAN → FAKTA
//         // =========================
//         const fakta = [];
//         let totalSkor = 0;
//         let adaNiatBunuhDiri = false;

//         for (const [kode, skor] of Object.entries(jawabanUser)) {
//             const nilai = parseInt(skor);
//             totalSkor += nilai;

//             if (nilai > 0) {
//                 fakta.push(kode); // gejala dianggap muncul
//             }

//             if (kode === 'C9' && nilai > 0) {
//                 adaNiatBunuhDiri = true;
//             }
//         }

//         // =========================
//         // 2. BASIS ATURAN
//         // =========================
//         const rules = [
//             {
//                 kode: 9,
//                 nama: "Depresi Ringan",
//                 gejala: ["C1","C2","C3"]
//             },
//             {
//                 kode: 10,
//                 nama: "Depresi Sedang",
//                 gejala: ["C1","C2","C3","C4","C6"]
//             },
//             {
//                 kode: 11,
//                 nama: "Depresi Mayor Berat",
//                 gejala: ["C1","C2","C3","C4","C5","C6","C7","C8","C9"]
//             }
//         ];

//         // =========================
//         // 3. FORWARD CHAINING
//         // =========================
//         const aturanTerpenuhi = rules.filter(rule =>
//             rule.gejala.every(g => fakta.includes(g))
//         );

//         // pilih aturan paling spesifik
//         let hasilAturan = null;
//         if (aturanTerpenuhi.length > 0) {
//             hasilAturan = aturanTerpenuhi.sort(
//                 (a, b) => b.gejala.length - a.gejala.length
//             )[0];
//         }

//         // =========================
//         // 4. VALIDASI MEDIS (PHQ-9)
//         // =========================
//         let hasilDiagnosa = '';
//         let saran = '';

//         if (totalSkor <= 4) {
//             hasilDiagnosa = 'Minimal / Normal';
//             saran = 'Kondisi kesehatan mental Anda tampak stabil.';
//         } else if (totalSkor <= 9) {
//             hasilDiagnosa = 'Depresi Ringan';
//             saran = 'Disarankan relaksasi dan pemantauan mandiri.';
//         } else if (totalSkor <= 14) {
//             hasilDiagnosa = 'Depresi Sedang';
//             saran = 'Disarankan berkonsultasi dengan profesional.';
//         } else if (totalSkor <= 19) {
//             hasilDiagnosa = 'Depresi Agak Berat';
//             saran = 'Sangat disarankan mencari bantuan profesional.';
//         } else {
//             hasilDiagnosa = 'Depresi Berat';
//             saran = 'Segera hubungi tenaga medis atau psikiater.';
//         }

//         // =========================
//         // 5. OVERRIDE ATURAN (LEBIH KUAT)
//         // =========================
//         if (hasilAturan) {
//             hasilDiagnosa = hasilAturan.nama;
//         }

//         // =========================
//         // 6. SAFETY OVERRIDE
//         // =========================
//         if (adaNiatBunuhDiri) {
//             hasilDiagnosa += ' (PERLU PERHATIAN KHUSUS)';
//             saran = '⚠️ Anda mengindikasikan pemikiran menyakiti diri. Segera hubungi layanan darurat atau profesional.';
//         }

//         return {
//             skor: totalSkor,
//             fakta,
//             diagnosis: hasilDiagnosa,
//             aturan_dipakai: hasilAturan?.gejala || [],
//             saran
//         };

//     } catch (error) {
//         console.error(error);
//         return {
//             diagnosis: "Error sistem",
//             saran: "-"
//         };
//     }
// }


// import { pool } from '../src/config/db.js';

// /**
//  * Logika Diagnosis PHQ-9
//  * @param {Object} jawabanUser - Object { D01: 2, D02: 0, ... }
//  */
// export async function forwardChaining(jawabanUser) {
//     try {
//         // 1. Hitung Total Skor
//         let totalSkor = 0;
//         let adaNiatBunuhDiri = false; // Red flag untuk D09

//         // Iterasi jawaban user
//         for (const [kode, skor] of Object.entries(jawabanUser)) {
//             totalSkor += parseInt(skor);
            
//             // Cek Red Flag (D09 > 0) -> Ini bahaya walau skor total rendah
//             if (kode === 'C9' && parseInt(skor) > 0) {
//                 adaNiatBunuhDiri = true;
//             }
//         }

//         // 2. Tentukan Kategori (Sesuai Standar Medis PHQ-9)
//         let hasilDiagnosa = '';
//         let saran = '';

//         if (totalSkor <= 4) {
//             // RANGE 0-4: INI BAGIAN PENTINGNYA!
//             hasilDiagnosa = 'Minimal / Normal (Tidak Ada Indikasi Depresi)';
//             saran = 'Kondisi kesehatan mental Anda tampak stabil dan sehat. Pertahankan pola hidup yang baik ini.';
            
//         } else if (totalSkor <= 9) {
//             // RANGE 5-9
//             hasilDiagnosa = 'Depresi Ringan (Mild Depression)';
//             saran = 'Anda mungkin sedang mengalami stres ringan. Coba lakukan relaksasi, olahraga, atau curhat ke teman dekat.';
            
//         } else if (totalSkor <= 14) {
//             // RANGE 10-14
//             hasilDiagnosa = 'Depresi Sedang (Moderate Depression)';
//             saran = 'Gejala mulai mengganggu aktivitas. Disarankan berkonsultasi dengan konselor atau psikolog untuk evaluasi.';
            
//         } else if (totalSkor <= 19) {
//             // RANGE 15-19
//             hasilDiagnosa = 'Depresi Agak Berat (Moderately Severe)';
//             saran = 'Gejala cukup signifikan. Sangat disarankan segera mencari bantuan profesional (Psikolog/Psikiater).';
            
//         } else {
//             // RANGE 20-27
//             hasilDiagnosa = 'Depresi Berat (Severe Depression)';
//             saran = 'Kondisi ini membutuhkan penanganan serius. Mohon segera hubungi dokter atau psikiater untuk bantuan medis.';
//         }

//         // 3. Override Bahaya (Safety First)
//         // Walaupun skor cuma 3 (Normal), tapi kalau D09 diisi, harus tetap warning.
//         if (adaNiatBunuhDiri) {
//             hasilDiagnosa += ' + (PERLU PERHATIAN KHUSUS)';
//             saran = '⚠️ PERINGATAN: Anda mengisi adanya pemikiran menyakiti diri. Abaikan hasil "Ringan/Normal" di atas. Segera hubungi layanan darurat, psikolog, atau orang terdekat.';
//         }

//         return {
//             skor: totalSkor,
//             hasil: hasilDiagnosa,
//             saran: saran
//         };

//     } catch (error) {
//         console.error(error);
//         return { hasil: "Error sistem", saran: "-" };
//     }
// }

// // import { pool } from '../src/config/db.js';

// // /**
// //  * Logika Diagnosis PHQ-9
// //  * @param {Object} jawabanUser - Object { D01: 2, D02: 1, ... } (Kode: Skor 0-3)
// //  */
// // export async function forwardChaining(jawabanUser) {
// //     try {
// //         // 1. Hitung Total Skor
// //         let totalSkor = 0;
// //         let adaNiatBunuhDiri = false;

// //         for (const [kode, skor] of Object.entries(jawabanUser)) {
// //             totalSkor += parseInt(skor);
            
// //             // Cek Red Flag (D09 > 0)
// //             if (kode === 'D09' && skor > 0) {
// //                 adaNiatBunuhDiri = true;
// //             }
// //         }

// //         // 2. Tentukan Kategori Berdasarkan Skor PHQ-9
// //         // 0-4: None, 5-9: Mild, 10-14: Moderate, 15-19: Moderately Severe, 20-27: Severe
// //         let hasilDiagnosa = '';
// //         let saran = '';

// //         if (totalSkor <= 4) {
// //             hasilDiagnosa = 'Tidak Ada Gejala Depresi Signifikan (Minimal)';
// //             saran = 'Kondisi mental Anda tampak stabil. Pertahankan pola hidup sehat.';
// //         } else if (totalSkor <= 9) {
// //             hasilDiagnosa = 'Depresi Ringan (Mild)';
// //             saran = 'Anda mungkin sedang stres. Coba lakukan relaksasi atau curhat ke teman dekat.';
// //         } else if (totalSkor <= 14) {
// //             hasilDiagnosa = 'Depresi Sedang (Moderate)';
// //             saran = 'Gejala mulai mengganggu. Disarankan berkonsultasi dengan konselor/psikolog.';
// //         } else if (totalSkor <= 19) {
// //             hasilDiagnosa = 'Depresi Agak Berat (Moderately Severe)';
// //             saran = 'Sangat disarankan untuk segera mencari bantuan profesional (Psikolog/Psikiater).';
// //         } else {
// //             hasilDiagnosa = 'Depresi Berat (Severe)';
// //             saran = 'Kondisi ini serius. Mohon segera hubungi dokter atau psikiater untuk penanganan medis.';
// //         }

// //         // 3. Tambahan Peringatan Keras jika ada D09
// //         if (adaNiatBunuhDiri) {
// //             hasilDiagnosa += ' + RISIKO MENYAKITI DIRI';
// //             saran = '⚠️ PERHATIAN: Anda melapor adanya pemikiran menyakiti diri. Segera hubungi layanan darurat atau pergi ke IGD terdekat.';
// //         }

// //         return {
// //             skor: totalSkor,
// //             hasil: hasilDiagnosa,
// //             saran: saran
// //         };

// //     } catch (error) {
// //         console.error(error);
// //         return { hasil: "Error sistem", saran: "-" };
// //     }
// // }


// // // import { pool } from '../src/config/db.js';
// // // import fs from 'fs';
// // // import path from 'path';

// // // /**
// // //  * Fungsi bantu: menulis log ke file dan terminal
// // //  */
// // // function writeLog(message) {
// // //     const logDir = path.resolve('logs');
// // //     if (!fs.existsSync(logDir)) fs.mkdirSync(logDir);

// // //     const logFile = path.join(logDir, 'forward.log');
// // //     const timestamp = new Date().toISOString();
// // //     const fullMsg = `[${timestamp}] ${message}\n`;
// // //     fs.appendFileSync(logFile, fullMsg);
// // //     console.log(message);
// // // }

// // // /**
// // //  * Fungsi utama inferensi forward chaining.
// // //  * @param {Array<string>} gejalaUser - array kode gejala yang dipilih user (contoh: ['D01', 'D02', 'A01'])
// // //  * @param {boolean} debug - aktifkan log debug
// // //  * @returns {Promise<string>} hasil diagnosa
// // //  */
// // // export async function forwardChaining(gejalaUser, debug = false) {
// // //     try {
// // //         if (debug) {
// // //             writeLog('\n🧩 [DEBUG MODE] Forward-Chaining Engine Aktif');
// // //             writeLog('🧠 Gejala User: ' + gejalaUser.join(', '));
// // //         }

// // //         // 1. CEK DATABASE (PRIORITAS TINGGI / RED FLAGS)
// // //         // Kita cek apakah ada rule "sakti" di database yang cocok 100%
// // //         const [rules] = await pool.query('SELECT * FROM aturan');
        
// // //         let hasilDiagnosa = null;
// // //         let ruleTerpakai = null;

// // //         for (const rule of rules) {
// // //             const kondisiRules = rule.kondisi.split(',').map((k) => k.trim());
            
// // //             // Cek apakah semua kondisi rule terpenuhi oleh gejala user
// // //             const isMatch = kondisiRules.every((kode) => gejalaUser.includes(kode));

// // //             if (isMatch) {
// // //                 hasilDiagnosa = rule.hasil;
// // //                 ruleTerpakai = `Rule ID ${rule.id} (DB)`;
// // //                 if (debug) writeLog(`🎯 Match Database Rule: ${rule.hasil}`);
                
// // //                 // Jika ketemu rule "BAHAYA" (misal D09 Bunuh Diri), langsung return, abaikan hitungan lain
// // //                 if (rule.hasil.includes('BAHAYA') || rule.hasil.includes('Sangat Berat')) {
// // //                     return hasilDiagnosa;
// // //                 }
// // //                 // Jika rule biasa, kita simpan dulu, tapi tetap jalankan evaluasi dinamis untuk kelengkapan
// // //                 break; 
// // //             }
// // //         }

// // //         // 2. JIKA TIDAK ADA MATCH DATABASE (ATAU TIDAK KRITIS), JALANKAN LOGIKA DSM-5
// // //         // Ini menangani logika "5 dari 9 gejala", "3 gejala fisik", dsb.
// // //         if (!hasilDiagnosa || !hasilDiagnosa.includes('BAHAYA')) {
// // //             const hasilDinamis = evaluasiDinamis(gejalaUser, debug);
            
// // //             // Jika database kosong, pakai hasil dinamis. 
// // //             // Jika database ada hasil tapi dinamis juga ada hasil (komorbid), gabungkan.
// // //             if (hasilDinamis) {
// // //                 hasilDiagnosa = hasilDiagnosa ? `${hasilDiagnosa} | ${hasilDinamis}` : hasilDinamis;
// // //                 ruleTerpakai = ruleTerpakai ? `${ruleTerpakai} + Algoritma DSM-5` : 'Algoritma DSM-5';
// // //             }
// // //         }

// // //         // Default jika sehat walafiat
// // //         if (!hasilDiagnosa) {
// // //             hasilDiagnosa = 'Tidak ditemukan indikasi gangguan mental klinis. Kesehatan mental Anda tampak stabil.';
// // //         }

// // //         if (debug) {
// // //             writeLog(`📝 Hasil Akhir: ${hasilDiagnosa}`);
// // //             writeLog('----------------------------------------\n');
// // //         }

// // //         return hasilDiagnosa;

// // //     } catch (error) {
// // //         writeLog(`❌ Error forwardChaining: ${error.message}`);
// // //         return 'Terjadi kesalahan sistem saat melakukan diagnosa.';
// // //     }
// // // }

// // // /**
// // //  * Logika Hitungan DSM-5 (Mayor Depresi, GAD, Burnout)
// // //  */
// // // function evaluasiDinamis(gejalaUser, debug) {
// // //     const diagnosaList = [];

// // //     // Filter gejala berdasarkan kategori
// // //     const gDepresi = gejalaUser.filter(k => k.startsWith('D')); // D01-D09
// // //     const gCemas = gejalaUser.filter(k => k.startsWith('A'));   // A01-A08
// // //     const gBurnout = gejalaUser.filter(k => k.startsWith('B')); // B01-B03

// // //     // --- 1. DEPRESI MAYOR (Major Depressive Disorder) ---
// // //     // Kriteria A: Minimal 5 gejala dalam 2 minggu
// // //     // Salah satu gejala HARUS D01 (Mood Depresi) atau D02 (Anhedonia)
// // //     const hasCoreDepression = gejalaUser.includes('D01') || gejalaUser.includes('D02');
// // //     const countDepression = gDepresi.length;

// // //     if (hasCoreDepression) {
// // //         if (countDepression >= 5) {
// // //             diagnosaList.push('Depresi Mayor (Indikasi Kuat)');
// // //         } else if (countDepression >= 2) {
// // //             diagnosaList.push('Depresi Ringan - Sedang (Gejala belum memenuhi kriteria penuh)');
// // //         }
// // //     } else if (countDepression >= 3) {
// // //         // Ada banyak gejala tapi tanpa mood sedih/hilang minat -> Mungkin gangguan lain
// // //         diagnosaList.push('Gangguan Mood Tidak Spesifik (Other Specified Depressive Disorder)');
// // //     }

// // //     // --- 2. GANGGUAN CEMAS MENYELURUH (GAD) ---
// // //     // Kriteria: Kecemasan berlebih (A01) DAN Sulit kontrol (A02)
// // //     // PLUS minimal 3 gejala fisik (A03-A08)
// // //     const hasCoreAnxiety = gejalaUser.includes('A01') && gejalaUser.includes('A02');
// // //     const countPhysicalAnxiety = gCemas.filter(k => ['A03','A04','A05','A06','A07','A08'].includes(k)).length;

// // //     if (hasCoreAnxiety && countPhysicalAnxiety >= 3) {
// // //         diagnosaList.push('Gangguan Kecemasan Menyeluruh (GAD)');
// // //     } else if (hasCoreAnxiety || countPhysicalAnxiety >= 3) {
// // //         diagnosaList.push('Gejala Kecemasan (Belum memenuhi kriteria penuh GAD)');
// // //     }

// // //     // --- 3. BURNOUT (Occupational Phenomenon) ---
// // //     // Kriteria: Dominan gejala B (biasanya semua 3 dimensi terpenuhi)
// // //     // Burnout murni biasanya terjadi jika kriteria Depresi/Cemas TIDAK terpenuhi
// // //     const countBurnout = gBurnout.length;
// // //     const isDepressed = diagnosaList.some(d => d.includes('Depresi'));

// // //     if (countBurnout >= 3 && !isDepressed) {
// // //         diagnosaList.push('Burnout (Sindrom Stres Akibat Pekerjaan)');
// // //     } else if (countBurnout >= 1 && isDepressed) {
// // //         diagnosaList.push('Gejala Burnout (Komorbid dengan Depresi)');
// // //     }

// // //     if (debug) {
// // //         writeLog(`   📊 Analisis: Depresi=${countDepression} (Inti:${hasCoreDepression}), Cemas=${countPhysicalAnxiety} (Inti:${hasCoreAnxiety}), Burnout=${countBurnout}`);
// // //     }

// // //     return diagnosaList.join(' DAN ');
// // // }


// // // // import { pool } from '../src/config/db.js';
// // // // import fs from 'fs';
// // // // import path from 'path';

// // // // /**
// // // //  * Fungsi bantu: menulis log ke file dan terminal
// // // //  */
// // // // function writeLog(message) {
// // // //     const logDir = path.resolve('logs');
// // // //     if (!fs.existsSync(logDir)) fs.mkdirSync(logDir);

// // // //     const logFile = path.join(logDir, 'forward.log');
// // // //     const timestamp = new Date().toISOString();
// // // //     const fullMsg = `[${timestamp}] ${message}\n`;
// // // //     fs.appendFileSync(logFile, fullMsg);
// // // //     console.log(message);
// // // // }

// // // // /**
// // // //  * Fungsi utama inferensi forward chaining.
// // // //  * @param {Array<string>} gejalaUser - daftar kode gejala yang dipilih user
// // // //  * @param {boolean} debug - aktifkan log debug & simpan ke file
// // // //  * @returns {Promise<string>} hasil diagnosa
// // // //  */
// // // // export async function forwardChaining(gejalaUser, debug = false) {
// // // //     try {
// // // //         if (debug) {
// // // //             writeLog('\n🧩 [DEBUG MODE] Forward-Chaining Engine Aktif');
// // // //             writeLog('🧠 Gejala yang dipilih user: ' + gejalaUser.join(', '));
// // // //         }

// // // //         // Ambil aturan dari database
// // // //         const [rules] = await pool.query('SELECT * FROM aturan');

// // // //         let hasilDiagnosa = null;
// // // //         let ruleTerpakai = null;

// // // //         // 🔁 Periksa setiap rule
// // // //         for (const rule of rules) {
// // // //             const kondisi = rule.kondisi.split(',').map((k) => k.trim());
// // // //             const cocok = kondisi.every((kode) => gejalaUser.includes(kode));

// // // //             if (debug) {
// // // //                 writeLog(`🔍 Mengecek aturan: [${kondisi.join(', ')}] → ${rule.hasil}`);
// // // //                 writeLog(`   → Status: ${cocok ? '✅ Cocok' : '❌ Tidak cocok'}`);
// // // //             }

// // // //             if (cocok) {
// // // //                 hasilDiagnosa = rule.hasil;
// // // //                 ruleTerpakai = rule.kondisi;
// // // //                 if (debug) writeLog(`🎯 Aturan cocok ditemukan → ${rule.hasil}`);
// // // //                 break;
// // // //             }
// // // //         }

// // // //         // 🔧 Jika tidak ada aturan cocok → fallback ke evaluasi dinamis
// // // //         if (!hasilDiagnosa) {
// // // //             hasilDiagnosa = evaluasiDinamis(gejalaUser, debug);
// // // //             ruleTerpakai = 'Evaluasi Dinamis (Fallback)';
// // // //         }

// // // //         if (debug) {
// // // //             writeLog(`\n🔎 Hasil akhir: ${hasilDiagnosa}`);
// // // //             writeLog(`🧩 Rule terpakai: ${ruleTerpakai}`);
// // // //             writeLog('----------------------------------------\n');
// // // //         }

// // // //         return hasilDiagnosa;
// // // //     } catch (error) {
// // // //         writeLog(`❌ Error dalam forwardChaining: ${error.message}`);
// // // //         return 'Terjadi kesalahan dalam proses diagnosa.';
// // // //     }
// // // // }

// // // // /**
// // // //  * Evaluasi dinamis berdasarkan jumlah gejala (jika tidak ada rule yang cocok)
// // // //  */
// // // // function evaluasiDinamis(gejalaUser, debug = false) {
// // // //     const jumlah = gejalaUser.length;
// // // //     const adaGejalaUtama = gejalaUser.includes('C1') || gejalaUser.includes('C2');
// // // //     let hasil;

// // // //     if (jumlah >= 5 && adaGejalaUtama) {
// // // //         hasil = 'Depresi Mayor (Kemungkinan Tinggi)';
// // // //     } else if (jumlah >= 3 && adaGejalaUtama) {
// // // //         hasil = 'Depresi Ringan–Sedang (Kemungkinan Sedang)';
// // // //     } else if (jumlah >= 3 && !adaGejalaUtama) {
// // // //         hasil = 'Gangguan Emosional Umum (Kemungkinan Rendah)';
// // // //     } else {
// // // //         hasil = 'Tidak terindikasi depresi mayor.';
// // // //     }

// // // //     if (debug) {
// // // //         writeLog('\n⚙️  Evaluasi Dinamis Aktif');
// // // //         writeLog(`   → Jumlah gejala: ${jumlah}`);
// // // //         writeLog(`   → Ada gejala utama (C1/C2): ${adaGejalaUtama ? 'Ya' : 'Tidak'}`);
// // // //         writeLog(`   → Kesimpulan: ${hasil}`);
// // // //     }

// // // //     return hasil;
// // // // }
